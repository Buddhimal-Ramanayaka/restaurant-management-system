...
# split on <w:p ...>, <w:p> and <w:p/> alike: paragraphs generated by the
# insertion scripts carry no attributes, and on the old pattern they were
# absorbed into the preceding chunk - which put several entries in one chunk
# and wrote the first entry's page number into the last entry's slot.
    ...
    if 'w:leader="dot"' in para:
        ...
        if len(ts)>=2 and re.fullmatch(r'[ivxlcdm\d]+', ts[-1].strip(), re.I):
            ...
            if lbl in FRONT: act=fh(FRONT[lbl])
                ...
                sec=re.match(r'^([\d.]+)',lbl).group(1); act=fh(r'^'+re.escape(sec)+r'\s+[A-Za-z]', True)   # headings may start lower case (eZee)
